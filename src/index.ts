...
const profileModel = require('./data')

var count: number = 0;
var countPath: string = 'count.txt';

mongoose
    .connect(process.env.MONGOOSE_DB, { useNewUrlParser: true, useFindAndModify: false, useUnifiedTopology: true })
...
bot.on('message', async function (msg) {
    ...
    for (var i = 0; i < channels.length; i++) {
        if (msg.channel.id === channels[i]) {
            ...
            let profileData;
            try {
                profileData = await profileModel.findOne({ userID: msg.author.id, serverID: msg.guild.id });
                if (!profileData) {
...
bot.on('message', async function (msg) {
    if (msg.guild === null) return;
    var msgLowercase = msg.content.toLowerCase();
    ...
    try {
        profileDataOther = await profileModel.findOne({ userID: msg.mentions.users.first().id, serverID: msg.guild.id });
    } catch (err) {
    }
    if (msgLowercase.includes('chair')) {
        ...
        const embed = new MessageEmbed()
            ...
            .setFooter('WARNING this is a beta your data might get deleted', profileData.pfp);
        ...
        try {
            const embed = new MessageEmbed()
                ...
                //.addField (`**${msg.author.username}'s chair count**`,`**chairs**: ${profileData.chair_count}`)
                .setFooter('WARNING this is a beta your data might get deleted', profileDataOther.pfp);
    ...
    else if (msgLowercase == '!leaderbord' || msgLowercase == '!lb' || msgLowercase == '!leaderbords') {
        leaderbord = await profileModel.find({ serverID: msg.guild.id })
        var embed = new MessageEmbed()
            .setFooter(`${profileData.serverName} | WARNING this is a beta your data might get deleted`, profileData.serverPFP,)
            .setColor('#78d6ff')
            .setTitle(`${profileData.serverName}'s Chair count`)

        leaderbord.sort((a: { chair_count: number; }, b: { chair_count: number; }) => b.chair_count - a.chair_count);
        ...
        var suggestion = msg.content.replace("!suggest", "");
        if (suggestion == "") {
...
const aboutEmbed = new MessageEmbed()
    .setColor('#78d6ff')
    .setDescription('this is my first bot i ever\nmade i hope you like the chairs :)')
    .setAuthor('Made by: Salty Mat', 'https://imgur.com/PWR3DeX.png', 'https://twitter.com/Salty_Mat')
    .setTitle('About')
    ...
    .addField(`contact me`, `**twitter**: @Salty_Mat\n**Youtube**: Salty Mat\n**Discord**: Salt#0330 `)
